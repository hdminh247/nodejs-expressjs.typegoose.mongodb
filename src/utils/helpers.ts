...
import BranchService from '../services/branch.io';
...
const helpers = {
    ...
    getPasswordUrl: async (code: string, type: string, role: string, tenantCode?: string): Promise<string> => {
    ...
    // Generate url for view request detail
    getJobRequestUrl: async (jobRequestId: string, tenantCode?: string): Promise<string> => {
        //Models
        const { jobRequest: jobRequestModel } = global.mongoModel;

        const webBaseUrlUser = global.env.webBaseUrlUser;

        const webPath = 'home/my-booking/current-bookings/detail/'

        const jobRequestData = await jobRequestModel.findOne({ _id: jobRequestId }).lean().exec();

        return (`${webBaseUrlUser}${webPath}${jobRequestData.job}`);
    },

    // Generate url for view recommend location
    getRecommendLocationUrl: async (jobId: string, tenantCode?: string): Promise<string> => {
        //Models
        const { job: jobModel } = global.mongoModel;

        const webBaseUrlUser = global.env.webBaseUrlUser;
        const webPath = 'home/booking?jobId='

        return (`${webBaseUrlUser}${webPath}${jobId}`);
    },
        ...
        if (isMultiple) {
            ...
            for (let i = 0; i < images.length; i++) {
                if (typeof images[i] === 'string') {
                    ...
                    images[i].path = images[i].path.replace(/ /g, '%20')
            ...
            if (typeof images === 'string') {
                ...
                images.path = images.path.replace(/ /g, '%20')
    ...
    sortArrayByFieldLength(arr: object[], sortType: number, field: string): object[] {
        arr.sort((a, b) => {

            let sortField = field.split('.'), aLength, bLength;
            ...
            return sortType > 0 ? (aLength < bLength ? -1 : (aLength > bLength ? 1 : 0)) : (aLength < bLength ? 1 : (aLength > bLength ? -1 : 0))
        })
    ...
    // Sort array by object field
    sortArrayByField(arr: object[], sortData: object): object[] {

        let sortBy = Object.keys(sortData)
        arr.sort((a, b) => {
            ...
            for (let i = 0; i < sortBy.length; i++) {
                let sortField = sortBy[i].split('.');
                a = data1
                b = data2

                for (let j = 0; j < sortField.length; j++) {

                    if (!a[sortField[j]] && b[sortField[j]]) {
                        return -sortData[sortBy[i]]
                    }

                    if (!b[sortField[j]] && a[sortField[j]]) {
                        return sortData[sortBy[i]]
                    }
                ...
                sortCondition.push({
                    a,
                    b,
                    sortType: sortData[sortBy[i]]
                })
            }
            return this.sortArray(sortCondition)
        })

        return arr
    },

    sortArray(sortCondition: any[]): number {
        for (let i = 0; i < sortCondition.length; i++) {
            if (sortCondition[i].a < sortCondition[i].b) return -sortCondition[i].sortType
            if (sortCondition[i].a > sortCondition[i].b) return sortCondition[i].sortType
        }
        return 0
    },
    ...
    deleteEmptyFieldInObject(data: object): object {
        // Field not have data
        let field = Object.keys(data);

        field.forEach(element => {
            if (data[element] === '' || data[element] === null || data[element] === undefined || data[element] === 'undefined') {
                delete data[element]
            }
        });
    ...
    // check valid keyword
    checkValidKeyword(keyword: string): boolean {
        const keywordRegex = /^[A-Za-z0-9 @.]+$/;

        return !!keywordRegex.exec(keyword)
    },

    // escape regexp
    escapeRegexp(keyword: string): any {
        if (!keyword) {
            return keyword;
        }
    ...
    findMinFieldInArrayObject(arr: object[], field: string): object {

        return arr.reduce((minValue, currentObj) => minValue[field] < currentObj[field] ? minValue : currentObj);
    },

    findMaxFieldInArrayObject(arr: object[], field: string): object {

        return arr.reduce((maxValue, currentObj) => maxValue[field] < currentObj[field] ? maxValue : currentObj);
    },
    ...
    paginateAnArray(arr: object[], size: number, page: number): object {
        ...
        let skip = 0, limit = 0, data = null, totalRow = 0;
    ...
    checkValueValidBaseOnRegion(region: string, value: string): boolean {
        const supportedCountry = ['anz', 'cn', 'id', 'my', 'nz', 'ph', 'sg', 'gb', 'us'];
